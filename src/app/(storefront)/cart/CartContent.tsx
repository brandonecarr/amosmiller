...
import { MembershipSelector } from "../checkout/MembershipSelector";

interface CartContentProps {
  isMember: boolean;
}

export function CartContent({ isMember }: CartContentProps) {
  const {
    items,
    updateQuantity,
    removeItem,
  ...
  const estimatedTotal = subtotal + membershipFee;

  const canProceedToCheckout = isMember || contractAccepted;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-slate-50 rounded-2xl p-6 sticky top-24">
            ...
            {/* Non-member notice when contract not yet accepted */}
            {!isMember && !contractAccepted && (
              <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                <p className="text-sm text-amber-800 font-medium">
                  Membership Required
                </p>
                <p className="text-xs text-amber-700 mt-1">
                  Please select a membership option and accept the membership contract below to proceed to checkout.
                </p>
              </div>
            )}
