...
} from "@/lib/actions/store-credits";
...
export default async function MemberDetailPage({
  ...
  const ordersPage = parseInt(sp.ordersPage || "1");
  const ordersPageSize = 10;
  const ordersOffset = (ordersPage - 1) * ordersPageSize;
  ...
  // Get total order count for this member
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { count: totalOrderCount } = await (supabase as any)
    .from("orders")
  ...
  const { data: ordersData } = await supabase
    .from("orders")
    .select("id, order_number, total, status, membership_fee, created_at")
    .eq("user_id", id)
    .order("created_at", { ascending: false })
    .range(ordersOffset, ordersOffset + ordersPageSize - 1);
  const orders: OrderData[] = ordersData || [];

  const ordersTotalPages = Math.ceil((totalOrderCount || 0) / ordersPageSize);

  // Get all orders for total spent calculation
  const { data: allOrdersData } = await supabase
    .from("orders")
    .select("total")
    .eq("user_id", id);
  const totalSpent = (allOrdersData || []).reduce(
  ...
  const creditHistory: StoreCreditTransaction[] =
              ...
              {ordersTotalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--color-border)]">
                  ...
                  <div className="flex gap-2">
                    {ordersPage > 1 && (
                      <Link
                        href={`/admin/members/${id}?ordersPage=${ordersPage - 1}`}
                      >
